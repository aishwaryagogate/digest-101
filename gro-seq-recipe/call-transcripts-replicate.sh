...
script_name="call-transcripts-replicate.sh"
script_ver="1.0.0"
...
usage() {
  ...
  echo "Example: $script_name -f 'foo_1.bam' -s 'foo_2.bam' -r 'hg19' -o '/path/to/output/dir/' -b -200 -u 5"
  exit 1
}
...
main(){
    ...
    while getopts :f:s:r:o:b:u:vh opt
        do
            case $opt in
                f) aln1=$OPTARG;;
                s) aln2=$OPTARG;;
    ...
    if [[ -z $aln1 ]] || [[ -z $aln2 ]] || [[ -z $ucsc_reference ]] || [[ -z $ltprob ]] || [[ -z $uts ]] || [[ -z $out ]]; then
        usage
    fi
    ...
    if [ ! -e $out_dir/metadata.json ]; then
        Rscript call-transcripts-replicate.R --replicate1 $aln1 --replicate2 $aln2 --genome $ucsc_reference --out $out_dir --ltprob $ltprob --uts $uts
        ...
        echo "* Transcripts has been made from $aln1_fn and $aln2_fn "
    fi
}
