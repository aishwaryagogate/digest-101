#!/bin/bash
# tune-hmm-timecourse.sh

script_name="tune-hmm-timecourse.sh"
script_ver="1.0.0"
...
usage() {
  ...
  echo "Example: $script_name -f 'foo_1.bam man_1.bam' -s 'foo_2.bam man_2.bam' -r 'hg19' -e 'foo man' -o '/path/to/output/dir/'"
  exit 1
}
...
main(){
    ...
    while getopts :f:s:r:e:o:vh opt
        do
            case $opt in
                f) aln1=$OPTARG;;
                s) aln2=$OPTARG;;
    ...
    if [ ! -e $out_dir/metadata.json ]; then
        Rscript tune-hmm-timecourse.R --replicate1 $aln1 --replicate2 $aln2 --experiment $exp --genome $ucsc_reference --out $out_dir
