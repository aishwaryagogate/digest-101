...
usage() {
  ...
  echo "Example: $script_name -f 'foo1.bam' -s 'foo2.bam' -e 'foo' -o '/path/to/output/dir/' -r 'hg19'"
  exit 1
}
...
main(){
    ...
    while getopts :f:s:o:e:r:vh opt
        do
            case $opt in
                f) aln1=$OPTARG;;
                s) aln2=$OPTARG;;
    ...
    if [[ -z $aln1 ]] || [[ -z $aln2 ]] || [[ -z $out ]] || [[ -z $experiment ]] || [[ -z $ucsc_reference ]]; then
        usage
    fi
    ...
    if [ ! -d $cross_correlation_dir ]; then
        ...
        Rscript cross-correlation-mod.R --replicate1 $aln1 --replicate2 $aln2 --genome $ucsc_reference --out $cross_correlation_dir
