...
call_peak() {
    ...
    fraglen=`$(head -n 1 filename) | cut -f3`
    echo $fraglen

    # Generate narrow peaks and preliminary signal tracks
    prefix=${experiment%.tagAlign.gz}
...
main(){
    ...
    if [ ! -e $out_dir/metadata.json ]; then
        ...
        pooled_replicates=$out_dir/experiment_pooled.tagAlign.gz
        gzip -dc $control | gzip -c > $out_dir/control_pooled.tagAlign.gz
        pooled_control=$out_dir/control_pooled.tagAlign.gz
        ...
        files=("$rep1", "$rep2", "$con1", "$con2". "$pooled_replicates")
        for f in $files;
            do
                ./pseudoreplicator.sh -f $f -o $out_dir
            done
        ...
        ./cross-correlation-only.sh -f $pooled_replicates -o $out
        pooled_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*.cc.qc`
        call_peak $pooled_replicates $pooled_control $pooled_xcor $chrom_sizes $genome_size $out_dir

        # Pseudoreplicates

        #Replicate1
        #Psuedo Replicate 1
        base_fn=$(basename "${rep1}")
        output_fp=${base_fn%.tagAlign.gz}
        rep1pr1=`ls $out_dir/pseudoreplicator.sh-1.0.0/$output_fp*.pr1.tagAlign.gz`
        ./cross-correlation-only.sh -f $rep1pr1 -o $out_dir
        rep1pr1_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr1.cc.qc`
        call_peak $rep1pr1 $con1 $rep1pr1_xcor $chrom_sizes $genome_size $out_dir

        #Psuedo Replicate 2
        base_fn=$(basename "${rep1}")
        output_fp=${base_fn%.tagAlign.gz}
        rep1pr2=`ls $out_dir/pseudoreplicator.sh-1.0.0/$output_fp*.pr2.tagAlign.gz`
        ./cross-correlation-only.sh -f $rep1pr2 -o $out_dir
        rep1pr2_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr2.cc.qc`
        call_peak $rep1pr2 $con1 $rep1pr2_xcor $chrom_sizes $genome_size $out_dir

        #Replicate2
        #Psuedo Replicate 1
        base_fn=$(basename "${rep2}")
        output_fp=${base_fn%.tagAlign.gz}
        rep2pr1=`ls $out_dir/pseudoreplicator.sh-1.0.0/$output_fp*.pr1.tagAlign.gz`
        ./cross-correlation-only.sh -f $rep2pr1 -o $out_dir
        rep2pr1_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr1.cc.qc`
        call_peak $rep2pr1 $con2 $rep2pr1_xcor $chrom_sizes $genome_size $out_dir

        #Psuedo Replicate 2
        base_fn=$(basename "${rep2}")
        output_fp=${base_fn%.tagAlign.gz}
        rep2pr2=`ls $out_dir/pseudoreplicator.sh-1.0.0/$output_fp*.pr2.tagAlign.gz`
        ./cross-correlation-only.sh -f $rep2pr2 -o $out_dir
        rep2pr2_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr2.cc.qc`
        call_peak $rep2pr2 $con2 $rep2pr2_xcor $chrom_sizes $genome_size $out_dir
        ...
        ./cross-correlation-only.sh -f $repppr1 -o $out_dir
        repppr1_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr1.cc.qc`
        call_peak $repppr1 $pooled_control $repppr1_xcor $chrom_sizes $genome_size $out_dir

        #Psuedo Replicate 2
        base_fn=$(basename "${pooled_replicates}")
        output_fp=${base_fn%.tagAlign.gz}
        repppr2=`ls $out_dir/pseudoreplicator.sh-1.0.0/$output_fp*.pr2.tagAlign.gz`
        ./cross-correlation-only.sh -f $repppr2 -o $out_dir
        repppr2_xcor=`ls $out_dir/cross-correlation-only.sh-1.0.0/$output_fp*pr2.cc.qc`
        call_peak $repppr2 $pooled_control $repppr2_xcor $chrom_sizes $genome_size $out_dir
