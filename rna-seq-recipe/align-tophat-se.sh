...
usage() {
  ...
  echo "Example: $script_name -f 'foo_1.fastq.gz' -i '/path/to/Bowtie2Index/genome' -a '/path/to/gtf/gencode.v19.annotation.gtf' -t '/path/to/gtf/index/gencode.v19.annotation' [-o '/path/to/output/dir/']"
  echo "Example: $script_name -f 'foo_1.fastq.gz,fastq2.fastq.gz' -i '/path/to/Bowtie2Index/genome' -a '/path/to/gtf/gencode.v19.annotation.gtf' -t '/path/to/gtf/index/gencode.v19.annotation' [-o '/path/to/output/dir/']"
  exit 1
}
...
main(){
    ...
    while getopts :f:i:a:t:o:vh opt
        do
            case $opt in
                f) raw_file=$OPTARG;;
                i) annotation=$OPTARG;;
    ...
    if [[ -z $raw_file ]] || [[ -z $annotation ]] || [[ -z $transcriptome ]] || [[ -z $transcriptome_index ]]; then
        usage
    fi
    ...
    aln_fn=$(basename "${raw_file_array[0]}")
    if [ ! -e $out_dir/$aln_fn.accepted_hits.bam ]; then
        ...
        tophat -p 8 --keep-fasta-order --no-coverage-search --library-type fr-firststrand -G $transcriptome \
            --transcriptome-index $transcriptome_index -g 10 --output-dir $out_dir $annotation $raw_file
        ...
        input_files=("$transcriptome" "$transcriptome_index" "$annotation" "${raw_file_array[@]}")
